...
JNIEXPORT jboolean JNICALL Java_com_tencent_yolov5ncnn_YoloV5Ncnn_Init(JNIEnv* env, jobject thiz, jobject assetManager)
{
    ...
    // init param
    {
        int ret = yolov5.load_param(mgr, "food.param");
        if (ret != 0)
        {
    ...
    // init bin
    {
        int ret = yolov5.load_model(mgr, "food.bin");
        if (ret != 0)
        {
...
JNIEXPORT jobjectArray JNICALL Java_com_tencent_yolov5ncnn_YoloV5Ncnn_Detect(JNIEnv* env, jobject thiz, jobject bitmap, jboolean use_gpu)
{
    ...
    static const char* class_names[] = {
            "麻婆豆腐","家常豆腐","土豆丝","手撕包菜",
            "炒青菜","炒豆芽","炒毛豆","炒西兰花","凉拌西红柿","炒山药","香菇青菜",
            "西红柿炒鸡蛋","韭菜炒鸡蛋","黄瓜炒鸡蛋","鸡蛋羹",
            "红烧肉"
    };
...
Java_com_tencent_yolov5ncnn_YoloV5Ncnn_openCamera(JNIEnv *env, jobject thiz, jint facing) {
